...
constexpr std::array<uint8_t, 2> kCandidateAddresses{0x3C, 0x3D};
...
esp_err_t Display::initialize(i2c_port_t port, gpio_num_t sda_pin, gpio_num_t scl_pin)
{
    port_ = port;
    i2c_config_t config{};
    ...
    ESP_RETURN_ON_ERROR(i2c_param_config(port_, &config), kTag, "I2C config failed");
    esp_err_t result = i2c_driver_install(port_, I2C_MODE_MASTER, 0, 0, 0);
    if (result != ESP_OK && result != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(kTag, "I2C driver install failed: %s", esp_err_to_name(result));
        return result;
    }

    ESP_RETURN_ON_ERROR(detect_address(), kTag, "SSD1306 address detection failed");
...
esp_err_t Display::detect_address()
{
    for (const uint8_t candidate : kCandidateAddresses) {
        ...
        if (result == ESP_OK) {
            result = i2c_master_write_byte(command, static_cast<uint8_t>((candidate << 1) | I2C_MASTER_WRITE), true);
        }
        if (result == ESP_OK) {
            result = i2c_master_stop(command);
        }
        if (result == ESP_OK) {
            result = i2c_master_cmd_begin(port_, command, kI2cTimeout);
        }
        i2c_cmd_link_delete(command);

        if (result == ESP_OK) {
            address_ = candidate;
            ESP_LOGI(kTag, "Detected SSD1306 at I2C address 0x%02X", address_);
            return ESP_OK;
        }
    }
...
esp_err_t Display::write_command(uint8_t command)
{
    ...
    return i2c_master_write_to_device(port_, address_, payload.data(), payload.size(), kI2cTimeout);
}
...
esp_err_t Display::write_data(const uint8_t* data, std::size_t length)
{
    ...
    while (length > 0) {
        ...
        ESP_RETURN_ON_ERROR(i2c_master_write_to_device(port_, address_, payload.data(), chunk + 1, kI2cTimeout), kTag,
